#!/usr/bin/env bash
# Functional verification for the reports-controller image — runs on the host under bash
...
[ -x /app/reports-controller ] || { echo "FAIL: /app/reports-controller is missing or not executable"; exit 1; }
echo "   /app/reports-controller present and executable"

echo "== --help smoke test (all that runs without a Kubernetes API) =="
/app/reports-controller --help >/tmp/help.out 2>&1
