...
# kyvernopre — upstream source compiled directly from a pinned commit (the v1.19.0 tag),
# reproducing kyverno's own Makefile `ko-build-kyverno-init`/`ko-publish-kyverno-init`
# targets (`CGO_ENABLED=0 go build ./cmd/kyverno-init` with the same ldflags) as a plain
# Dockerfile build instead of `ko`. Note the source directory is `cmd/kyverno-init`, not
# `cmd/kyvernopre` — the image/repo name `kyvernopre` differs from the upstream source
# directory name; the output binary itself is named `kyvernopre` (Makefile's
...
#     is `/app/kyvernopre`. kyverno's Helm chart does not hardcode a `command:` for the
#     init container, only `args:`, so this does not change chart behaviour.
...
#     `Time()` fall back to "---"; BuildVersion (ldflags below) is what verify.sh and the
...
# Reproduces upstream Makefile's ko-build-kyverno-init target as-is (GOARCH replaced by
...
RUN --mount=type=cache,target=/root/go/pkg/mod \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=${TARGETARCH} \
      go build -trimpath \
        ...
        -o /out/kyvernopre ./cmd/kyverno-init

FROM ${RUNTIME_BASE} AS final

WORKDIR /app
COPY --from=builder /out/kyvernopre ./kyvernopre
