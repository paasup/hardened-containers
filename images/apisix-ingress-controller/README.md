...
| Transitive dependencies | The versions pinned in go.mod | The `GO_MODULE_UPGRADES` list raised to its minimum compatible versions with `go get` plus `go mod tidy` | This is a single-module project with no `go.work` workspace, so one global `replace` line is not enough — several modules must be specified together (otel core and sdk in particular must have matching versions), and `go get` settles the remaining compatible versions. This replaced per-module `<LIB>_FIX_VERSION` ARGs: `suggest-go-upgrades.py --apply` never adds a missing key, so keeping them would have left this image out of the automated fix |
