#!/usr/bin/env bash
# Functional verification for the cleanup-controller image — runs on the host under bash
...
[ -x /app/cleanup-controller ] || { echo "FAIL: /app/cleanup-controller is missing or not executable"; exit 1; }
echo "   /app/cleanup-controller present and executable"

echo "== --help smoke test (all that runs without a Kubernetes API) =="
/app/cleanup-controller --help >/tmp/help.out 2>&1
