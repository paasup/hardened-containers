#!/usr/bin/env bash
# Functional verification for the background-controller image — runs on the host under
# bash (build-hardened-image.sh invokes it as `env TAG=... PLATFORM=... SOURCE_COMMIT=...
# APP_VERSION=... bash verify.sh`).
#
# Starting the controller for real requires a Kubernetes API server connection, which is
# outside this smoke test's scope (see README.md). Only what can be checked without one is
...
[ -x /app/background-controller ] || { echo "FAIL: /app/background-controller is missing or not executable"; exit 1; }
echo "   /app/background-controller present and executable"

echo "== --help smoke test (all that runs without a Kubernetes API) =="
/app/background-controller --help >/tmp/help.out 2>&1
