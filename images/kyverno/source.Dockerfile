...
# kyverno — upstream source compiled directly from a pinned commit (the v1.19.0 tag),
# reproducing kyverno's own Makefile `ko-build-kyverno`/`ko-publish-kyverno` targets
# (`CGO_ENABLED=0 go build ./cmd/kyverno` with the same ldflags) as a plain Dockerfile
# build instead of `ko`.
#
...
#     is `/app/kyverno`. kyverno's Helm chart does not hardcode a `command:`, only `args:`,
#     so this does not change chart behaviour.
#   - No `.git` dir kept on the git ADD (see README.md "Building and verifying" for why) —
...
# Reproduces upstream Makefile's ko-build-kyverno target as-is (GOARCH replaced by
...
RUN --mount=type=cache,target=/root/go/pkg/mod \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=${TARGETARCH} \
      go build -trimpath \
        ...
        -o /out/kyverno ./cmd/kyverno

FROM ${RUNTIME_BASE} AS final

WORKDIR /app
COPY --from=builder /out/kyverno ./kyverno
