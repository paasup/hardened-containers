#!/usr/bin/env bash
# Functional verification for the kyverno image — runs on the host under bash
...
[ -x /app/kyverno ] || { echo "FAIL: /app/kyverno is missing or not executable"; exit 1; }
echo "   /app/kyverno present and executable"

echo "== --help smoke test (all that runs without a Kubernetes API) =="
/app/kyverno --help >/tmp/help.out 2>&1
